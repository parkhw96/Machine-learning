...
- ### <u>sequence</u>
  ...
  ##### 4. 반복 횟수 만큼 gradient descent를 통해 $\theta_0$와 $\theta_1$의 값을 변경, 이때 매번 cost function의 값을 계산

  ##### 5. 반복 횟수 만큼 끝나게 되면 $\theta_0$와 $\theta_1$의 값이 어떠한 값으로 수렴하게 되는데, 이 때의 값이 cost function을 최소화 하는 값이다.
